...
const calcAddedBal = document.querySelector('.add-bal-btn');
const initialBalAdd = document.querySelector('.total-output-bal #total-amount-output-bal');
const keypads = document.querySelectorAll('.keypad-item');
...
// Displays Storage Balance
if (localStorage.getItem('balance')) {
    calcValue = parseFloat(localStorage.getItem('balance'));
    updateBalanceDisplay(); 
}

// Adds Expense Items (i.e. Jeepney Rides - P11)
...
// Adds Custom Expense Input (i.e Enter Amount)
...
// Adds Balance
calcAddedBal.addEventListener('click', () => {
...
// Adds Balance Through Keypad
keypads.forEach(button => {
  button.addEventListener('click', () => {
    const value = button.textContent.trim();

    if (button.id === 'backspace')
      addBalInpt.value = addBalInpt.value.slice(0, -1);

    addBalInpt.value += value;
  });
});
...
window.addEventListener('load', () => {
    ...
    document.querySelector('.no-trans').textContent = 'No Transactions Yet!';
    return;
    }
